...
import {
	getManagersIndexBundle,
	getManagerDossierBundle
} from '$lib/server/league/managerDossiers.js';
...
import { getFranchiseTransactionHistory } from '$lib/server/league/franchiseTransactionHistory.js';

async function getCareerDraftCapitalStats(db, managerId) {
	const empty = {
		draftCapitalSent: 0,
		draftCapitalAcquired: 0,
		draftCapitalNet: 0,
		draftCapitalTransfers: 0
	};
	...
	const row = await db
      ...
      `
		...
		.bind(String(managerId))
		.first();

	const sent = Number(row?.sent_cents || 0) / 100;

	const acquired = Number(row?.acquired_cents || 0) / 100;

	return {
		draftCapitalSent: sent,

		draftCapitalAcquired: acquired,

		draftCapitalNet: acquired - sent,

		draftCapitalTransfers: Number(row?.movement_count || 0)
	};
}
...
export async function getTeamDetailBundle({
  url,
  env,
  slug
} = {}) {
  ...
  /*
   * Sleeper transaction history and
   * Irving draft-capital history are
   * intentionally independent sources.
   */
  const moveProfile =
    transactionHistory.hasData
      ? transactionHistory.moveProfile
      : dossier.moveProfile;

  const baseTradeProfile =
    transactionHistory.hasData
      ? transactionHistory.tradeProfile
      : dossier.tradeProfile;
  ...
  const tradeProfile = {
    ...
    /*
     * ALWAYS use D1 for these.
     *
     * Do not let a Sleeper-history fallback
     * overwrite capital totals.
     */
    ...capitalStats
  };
  ...
  return {
    ...
    franchise: {
      ...
      teamName:
        dossier.manager.liveTeamName,

      teamPhoto:
        dossier.manager.photo,

      managerName:
        dossier.manager.name,

      bio:
        dossier.manager.bio,

      currentRank:
        dossier.manager.currentRank,

      recordLabel:
        dossier.manager.recordLabel,

      pointsFor:
        dossier.manager.pointsFor,

      pointsAgainst:
        dossier.manager.pointsAgainst,

      pointDiff:
        dossier.manager.currentPointDiff,

      favoriteTeam:
        dossier.manager.favoriteTeam,

      philosophy:
        dossier.manager.philosophy,

      preferredContact:
        dossier.manager.preferredContact,

      championship:
        dossier.manager.championship
    },
    ...
    sections: {
      dossier:
        `/league/managers/${slug}?season=${dossier.season}`,

      moves:
        `/league/transactions?season=${dossier.season}&team=${slug}`,

      games:
        `/league/matchups?season=${dossier.season}&team=${slug}`,

      drafts:
        `/league/drafts?season=${dossier.season}`,

      standings:
        `/league/standings?season=${dossier.season}`
    },
...
export async function getTeamWeeklyBundle({ url, env, slug, week } = {}) {
	...
	const roster =
		rosters.find((row) => String(row.owner_id || '') === String(profile.managerID)) || null;
	...
	const opponent = side
		? (matchups || []).find(
				(item) => item.matchup_id === side.matchup_id && Number(item.roster_id) !== rosterId
			)
		: null;
	...
	const lineupSnapshot = buildSingleWeekLineupSnapshot({
		week: selectedWeek,
		side,
		roster,
		league,
	...
	const previousWeek =
		context.availableWeeks.filter((value) => value < selectedWeek).at(-1) || null;
