...
async function derivePasswordBytes(password, saltBytes, iterations) {
	...
	if (
		!Number.isFinite(safeIterations) ||
		safeIterations <= 0 ||
		safeIterations > MAX_ITERATIONS
	) {
		throw new Error(
			`Unsupported PBKDF2 iteration count: ${iterations}`
		);
	}
...
export async function hashPassword(
	...
	const safeIterations =
		...
		requestedIterations > 0
			? Math.min(requestedIterations, MAX_ITERATIONS)
			: DEFAULT_ITERATIONS;

	const salt = new Uint8Array(SALT_BYTES);
	crypto.getRandomValues(salt);

	const hash = await derivePasswordBytes(
		password,
		salt,
		safeIterations
	);
