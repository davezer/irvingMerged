...
<div class="page-stack draft-page">
	...
	<section class="draft-hero bug-panel">
		...
		<div class="draft-hero-main icl-hero-shell pad-md">
			<div>
			...
			<div class="season-box">
				...
				<div class="season-pills">
					{#each archive as item}
						<a class:selected={item.season === data.season} href={`?season=${item.season}`}
							>{item.season}</a
						>
					{/each}
				</div>
						...
						{#each teamBoards as team (team.teamName)}
							<a
								...
								href={team.managerSlug
									? `/league/teams/${team.managerSlug}?season=${data.season}`
									: `/league/teams?season=${data.season}`}
							>
								<div class="team-logo">
									{#if team.teamPhoto}
...
<style>
	...
	.team-card {
		...
		background:
			linear-gradient(180deg, rgba(255, 255, 255, 0.1), rgba(255, 255, 255, 0.02) 20%),
