...
import { getHomepageStats, getManagers } from '$lib/server/league';

import { getLiveStandingsBundle } from '$lib/server/league/standings.js';

import { listPublishedWeeklyPosts } from '$lib/server/league/weeklyPostRepository.js';
...
async function getCollectiveSnapshot(db, user) {
	...
	try {
		...
		const eventsUpcoming = eventsAll
			.filter((event) => (event.start_at || event.lock_at || 0) >= now - 60 * 60 * 24)
			.slice(0, 4);
		...
		console.warn('Homepage collective snapshot unavailable', error);

		return empty;
	}
}

async function getLatestIrvingWeekly(db) {
	if (!db) {
		return [];
	}

	try {
		return await listPublishedWeeklyPosts(db, {
			limit: 3
		});
	} catch (error) {
		console.warn('Homepage Irving Weekly unavailable', error);
...
export async function load({
	...
	const [
		...
		await Promise.all([
			getCollectiveSnapshot(
				db,
				locals.user
			),

			getLatestIrvingWeekly(
				db
			),
		...
		commandCards: [
			{
				...
				body:
					'Standings, rosters, matchups, drafts, transactions, manager dossiers, and the dumb prestigious history that makes this league this league.',
...

			{
				...
				body:
					'Masters panic, Derby double, March Madness, Race Crash Cash, World Cup chaos, leaderboard pressure, and fresh ways to lose dignity.',
...

			{
				...
				title:
					'History, Badges & Rivalries',

				body:
					'Legacy titles, personas, stains, grudges, records, and the permanent paper trail of every manager’s worst choices.',
