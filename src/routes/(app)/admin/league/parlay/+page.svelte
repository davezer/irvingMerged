...
<div class="shell">
  ...
  <section class="workspace-grid">
    ...
    <aside class="side-column">
      ...
      {#if data.selectedWeek}
        <details class="panel utility manual-entry" open={missingManagers.length === 1}>
          <summary>
            <div>
              <p class="kicker gold">COMMISSIONER ENTRY</p>
              <h2>Manual pick</h2>
            </div>
            <span>+ ADD</span>
          </summary>

          <p>Add a leg for a manager who is not submitting through Discord. It counts exactly like a normal weekly pick.</p>

          <form method="POST" action="?/addManualPick" class="manual-form">
            <input type="hidden" name="weekId" value={data.selectedWeek.id} />
            <input type="hidden" name="season" value={data.selectedWeek.season} />
            <input type="hidden" name="week" value={data.selectedWeek.week} />

            <label>
              Manager
              <select name="managerId" required>
                <option value="">Choose manager…</option>
                {#each missingManagers as manager}
                  <option value={manager.id}>{manager.teamName} · {manager.name}</option>
                {/each}
              </select>
            </label>
            ...
            <label>Player / Team<input name="subject" placeholder="Saquon Barkley or Giants" required /></label>
            <label>Market<input name="market" placeholder="Rushing yards, spread, total, etc." /></label>
            ...
            <div class="three">
              ...
              <label>Line<input name="line" type="number" step="0.5" placeholder="74.5" /></label>
              <label>Odds<input name="odds" placeholder="-110" required /></label>
            </div>

            <label>Notes<textarea name="notes" placeholder="Optional commissioner note"></textarea></label>
            <button class="manual-submit">Add pick to current ticket</button>
          </form>
        </details>
      {/if}
