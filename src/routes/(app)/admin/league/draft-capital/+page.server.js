...
importLegacyLedger: async ({
  ...
  try {
    ...
    /*
     * We have mathematical proof that this CSV rebuilds
     * the exact board currently shown in D1.
     */
    const result =
    ...
    /*
     * One final verification after the swap.
     */
    const after =
      await getDraftCapitalBalances(
        db,
    ...
    if (
      ...
      return fail(
        500,
        {
          ...
          error:
            `Historical rows were imported, but the final verification failed for: ${postImportErrors.join(', ')}`
        }
      );
    }
    ...
    return {
      ...
      message:
        `Legacy ledger imported successfully: ${result.storedRows} historical rows, ${tradeRows / 2} reconstructed trades, and all 14 balances verified exactly.`
    };
